            ...
            function function3 (element){                               //start of function 
                ...
                var link = document.querySelectorAll('a');
                    if (elementSize.top <= 171) {                       // make condition by 
                            ...
                            //active link
                                 link.forEach(function5);
                                    function function5 (ancho){ 
                                        if (element.getAttribute("id") == ancho.getAttribute('data-nav')) {ancho.classList.add('activeclass');}
                                        else {ancho.classList.remove('activeclass')};
...
window.addEventListener('scroll', function (){
    var toppage = document.querySelector('.topbutton');
...
var buttonpage = document.querySelector('.topbutton');
...
window.addEventListener('scroll',function(){
    ...
    var react = document.documentElement.getBoundingClientRect();
    
    if (react.top == 0 && react.left == 0){
//        console.log(document.documentElement.scrollTop);
        links.forEach(fun);
        function fun (ele){
        if (ele.classList.contains('activeclass')) {ele.classList.remove('activeclass')}
}
    }
